...
const Post = (props) => {
  const [blog, setBlog] = useState(props.article[0]);
  ...
  return (
    <div className={styles.post}>
      ...
      <div className={styles.head}>
        ...
        <div className={styles.date}>{blog && getFullDate(blog.date)}</div>
      </div>
      <div className={styles.blog}>
        ...
        {/* {blog &&
          blog.parts &&
          blog.parts.map((e, idx) => <span key={idx}>{e}</span>)} */}
        <PortableText
          ...
          serializers={{
            ...
            li: ({ children }) => (
              <li
                className="mx-2 my-4 tracking-wide"
                style={{ fontSize: "1.07rem", wordSpacing: "2px" }}
              >
                {children}
              </li>
            ),
        ...
        <div className={styles.ending}>
          ...
          <div className="flex justify-between flex-wrap">
            ...
            <Link href="/articles">
              <a className={`${styles.share} tracking-wider`}>
...
export async function getServerSideProps(context) {
  ...
  const query = `*[_type == "articles" && slug.current == '${slug}']`;
